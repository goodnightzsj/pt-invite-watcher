    ...
    <div v-if="site" class="space-y-6">
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
          <Badge label="连通性"
            :tone="site.reachability_state === 'up' ? 'green' : site.reachability_state === 'down' ? 'red' : 'slate'" />
          <Badge label="注册"
            :tone="site.registration_state === 'open' ? 'green' : site.registration_state === 'closed' ? 'amber' : 'slate'" />
          <Badge label="邀请"
            :tone="site.invites_state === 'open' ? 'brand' : site.invites_state === 'closed' ? 'slate' : 'slate'" />
        </div>
      </div>
      ...
      <div v-if="site.errors && site.errors.length > 0">
        ...
        <div
          class="max-h-60 overflow-y-auto rounded-xl border border-danger-200 bg-danger-50 p-3 text-xs font-mono text-danger-900 dark:border-danger-900 dark:bg-danger-950/40 dark:text-danger-200">
          <div v-for="(err, idx) in site.errors" :key="idx"
            class="mb-1 border-b border-danger-200/50 pb-1 last:border-0 last:pb-0">
            {{ err }}
          </div>
        </div>
      </div>
