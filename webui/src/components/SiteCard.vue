...
<template>
  <div 
    class="relative overflow-hidden rounded-xl bg-white p-4 shadow-sm transition-all active:scale-[0.98] dark:bg-slate-900/50 glass hover:shadow-md"
    @click="emit('click')"
  >
    ...
    <!-- Footer: Time -->
    <div class="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-xs text-slate-400 dark:border-slate-800">
        <div>{{ site.engine }}</div>
        <div>{{ site.last_checked_at.replace('T', ' ').split('.')[0] || '-' }} 检测</div>
    </div>
