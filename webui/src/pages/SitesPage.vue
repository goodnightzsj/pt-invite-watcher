...
<template>
  <div class="space-y-5">
    ...
    <EmptyState v-if="items.length === 0 && !loading" title="暂无站点" description="请检查 MoviePilot 连接配置或手动添加站点"
      actionText="新增站点" @action="openAdd" />
    ...
    <Modal :open="modalOpen" :title="modalTitle" @close="modalOpen = false">
      <div class="space-y-4">
        ...
        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40">
          ...
          <div class="space-y-3">
            <FormInput label="Cookie Header（留空不修改）" v-model="form.cookie" type="textarea"
              placeholder="uid=...; pass=...; ..." :disabled="form.clear_cookie" />
            <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
              <input v-model="form.clear_cookie" type="checkbox"
        ...
        <div v-if="isMteam"
          ...
          <div class="mb-3 flex items-center justify-between gap-3">
            ...
            <div class="flex flex-wrap gap-2">
              <Badge :label="form.authorization_configured ? 'Authorization 已配置' : 'Authorization 未配置'"
                :tone="form.authorization_configured ? 'green' : 'amber'" />
              <Badge :label="form.did_configured ? 'API Key 已配置' : 'API Key 未配置'"
                :tone="form.did_configured ? 'green' : 'amber'" />
            </div>
          </div>
          <div class="space-y-3">

            <FormInput label="Authorization（留空不修改）" v-model="form.authorization" type="textarea"
              placeholder="eyJhbGciOiJIUzUxMiJ9..." :disabled="form.clear_authorization" />
            <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
              <input v-model="form.clear_authorization" type="checkbox"
                class="h-4 w-4 rounded border-slate-300 text-slate-900 dark:border-slate-700 dark:bg-slate-950" />
              清空 Authorization
            </label>

            <FormInput label="API Key（did，留空不修改）" v-model="form.did" placeholder="63788714-a1fa-..."
              :disabled="form.clear_did" />
            <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
              <input v-model="form.clear_did" type="checkbox"
                class="h-4 w-4 rounded border-slate-300 text-slate-900 dark:border-slate-700 dark:bg-slate-950" />
              清空 API Key
            </label>
          </div>
